...
```sh
Cân bằng tải theo cách này sẽ chuyển hướng lưu lượng người dùng dựa trên IP range và port.
vd: nếu 1 request đến http://yourdomain.com/anything, lưu lượng sẽ được chuyển hướng đến backend mà xử lý tất cả các request cho yourdomain.com trên port 80)
```
...
###4. Thuật toán Fastest (Thuật toán Least Response Time)
...
###6. Thuật toán Weights Least Connection:
...
Ngoài HAProxy (Layer 4) còn có các giải pháp sau đây:
